...
export function PromoPopup() {
  ...
  const code = settings["promo_code"] || "";
  const headline = settings["promo_headline"] || "Welcome! Here is a special offer just for you";
  const subtext = settings["promo_subtext"] || "Use code " + code + " at checkout.";
  const badge = settings["promo_badge"] || "SPECIAL OFFER";
  const imageUrl = settings["promo_image_url"] || "";

  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center p-4 sm:items-center">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={dismiss} />

      <div className={"relative w-full overflow-hidden rounded-2xl bg-white shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300 sm:rounded-3xl " + (imageUrl ? "max-w-lg" : "max-w-sm")}>
        <div className={"flex " + (imageUrl ? "flex-col sm:flex-row" : "flex-col")}>
          ...
          {imageUrl && (
            <div className="relative sm:w-44 sm:shrink-0">
              <div className="h-48 sm:h-full">
                ...
                />
                <div className="absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-white to-transparent sm:hidden" />
              </div>
            </div>
          )}
